...
future<> TCPServer::accept() {
    listener.accept().then([this] (net::tcp<ipv4_traits>::connection conn) {
...
future<> TCPConnectionHandler::handle() {
        connection.wait_for_data().then([this] {
        ...
        connection.send(std::move(bytes_read));
        handle();
    });
}
